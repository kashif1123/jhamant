    ...
    <script>
        ...
        $(document).ready(function (e) {
            ...
            editor1 =  new $.fn.DataTable.Editor({
                ajax: "{{ url('supplier/dtpostshowsupplier') }}",
                table: "#products",
                display: "bootstrap",
                idSrc:'id',
                fields: [
                    {label: "Name:", name: "name",type:"readonly"},
                    {label: "CNIC:", name: "cnic",type:"readonly"},
                    {label: "Address:", name: "address"},
                    {label: "Opening Balance:", name: "opening_balance",type:"readonly"},
            ...
            //     console.log(d);
                ...
                //     '</tr>' +
                //     '</tbody></table>';
                // table2 += '<table border="0" style="padding-left:50px;" class="table">'+
                //     '<thead><tr><th>Sr</th><th>Contact Number</th></tr></thead>';
                // for (var i=0;i<d.data.length;i++) {
                //     table2+='<tr>' +
                //         '<td>'+(i+1)+'</td>' +
                //         '<td>'+d.data[i].contact+'</td>' +
                //         '</tr>' ;
            ...
            $('#products tbody').on('click', 'td.fa-plus-circle', function () {
                ...
                if ( row.child.isShown() ) {
                    ...
                    // Open this row
                    $.ajax({
                        url:'{{ url('supplier/dtgetshowsuppliercontacts') }}',
                        type:'get',
                        data:{'id':row.data().id},
            ...
            var table=  $('#products').DataTable({
                ...
                buttons: [
                    ...
                    { extend: "edit",   editor: editor1 ,className:'m-btn--square  btn-outline-warning'},
                    { extend: "remove", editor: editor1 ,className:'m-btn--square  btn-outline-danger'},
                    {'extend': 'print', className: 'm-btn--square  btn-outline-primary',
                        ...
                        title: function(){
                            var printTitle = '' +
                                ...
                                '                                        <h1 class=" mr-2" style="font-family: sans-serif;text-decoration: underline">All Users</h5>\n' +
                                '                                    </div>\n' +
                                '                                    <div class="col-md-6">\n' +
            ...
            $(document).on('click','.editor_remove',function (e) {
                ...
                $.ajax({
                    url: '{{ url("supplier/delete_supplier") }}',
                    type: 'post',
                    data: {
                        'id': data.id,
                        'opening_blance': data.opening_balance,
                        '_token': '{{csrf_token()}}'
                    },
                    success: function (data) {
                        console.log(data);
            ...
            {{--var table2=  $('#contacts').DataTable({--}}
            {{--    dom: "Blfrtip",--}}
            {{--    "bprocessing": true,--}}
            {{--    "bserverSide": true,--}}
            {{--    "ajax": '{{ url('supplier/dtget_s_contacts') }}',--}}
            {{--    // "columnDefs": [--}}
            {{--    //     { "targets": [1,2,3,4], "searchable": false }--}}
            {{--    // ],--}}
            {{--    "paging"     : true,--}}
            ...
            {{--        {'extend': 'print', className: 'm-btn--square  btn-outline-primary',title: function(){--}}
            {{--                var printTitle = '' +--}}
            {{--                    '<div class="col-md-12 alert" style="background-color: rgba(0,0,0,0.55);color: white;">\n' +--}}
            {{--                    '                                    <div class="col-md-6 pl-4">\n' +--}}
            {{--                    '                                        <h4 class="" style="font-family: sans-serif;">Chief Executive</h4>\n' +--}}
            {{--                    '                                        <h4 class="m-0" style="font-family: sans-serif;">Haji Mukhtar Ahmed</h4>\n' +--}}
            {{--                    '                                        <h5 class=" mr-2" style="font-family: sans-serif;">Secretary Fertilizers Dealers Association Sargodha Devision</h5>\n' +--}}
            {{--                    '                                    </div>\n' +--}}
            {{--                    '                                    <div class="col-md-6">\n' +--}}
            {{--                    '                                        <h5 class="ml-4 m-0" style="font-family: sans-serif;">Fertilizers Dealers</h5>\n' +--}}
            {{--                    '                                        <h2 class="ml-4 m-0" style="font-family: sans-serif;">Hussain Traders (048-3713181)</h2>\n' +--}}
            {{--                    '                                        <h5 class="ml-4" style="margin-bottom: 30px; font-family: sans-serif;">0300-6046015,0345-8601433,0322-8600598</h5>\n' +--}}
            {{--                    '                                        <small class="ml-4" style="font-style:italic;float: right;  font-size: 12px;  font-family: sans-serif;">Printing Date: '+ new Date().toJSON().slice(0,10).replace(/-/g,'/') +' (A Software By BRAINIACCS 03032600069) </small>\n' +--}}
            {{--                    '                                    </div>\n' +--}}
            {{--                    '\n' +--}}
            {{--                    '                                    <div>\n' +--}}
            {{--                    '                                    </div>\n' +--}}
            ...
            //     submitOnBlur: true
            // })
            $('#contacts').on('click', 'tbody td:not(:first-child)', function (e) {
                editor.inline(this);
            });
