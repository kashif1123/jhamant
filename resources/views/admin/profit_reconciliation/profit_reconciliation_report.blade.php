                ...
                <div class="kt-portlet kt-portlet--last kt-portlet--head-lg kt-portlet--responsive-mobile" id="kt_page_portlet">
                    <div class="kt-portlet__head kt-portlet__head--lg">
                        <div class="kt-portlet__head-label">
                            <h3 class="kt-portlet__head-title">Profit Reconciliation Report</h3>
                        </div>
...
{{--                                        <div class="col-md-2">--}}
{{--                                            <label>To</label>--}}
{{--                                            <input name="todate" style="background-color: white !important;" readonly id="kt_datepicker_1" class="form-control max c_l_o_s_e js-example-basic-single" placeholder="To Date" type="text" >--}}

{{--                                        </div>--}}
{{--                                        <div class="col-md-2" style="margin-top: 26px;">--}}
{{--                                            <button class="btn btn-primary btn-sm form-control loaddatabydate">Submit</button>--}}
{{--                                        </div>--}}
{{--                                    </div>--}}

                                </div>
                                <table id="installmentplan" class="table table-bordered table-striped">
                                    <thead>
                                    <tr>
                                        <th>Sr</th>
                                        <th>Month</th>
                                        <th>Profit/Loss</th>
                                        <th>Month Total Profit/Loss</th>
                                        <th>Month Total Reconciled</th>
                                        <th>Owner</th>
                                        <th>Amount Reconciled</th>
                                        <th>Reconcile Description</th>
...
@section('scripts')
    ...
    <script>
        ...
        $(document).ready(function (e) {
            ...
            {{--    ajax: "{{ url('profit_reconciliation/dt_get_all_reconciliations') }}",--}}
            {{--    table: "#installmentplan",--}}
            {{--    display: "bootstrap",--}}
            {{--    idSrc:'id',--}}
            {{--    fields: [--}}
            {{--        // {label: "Expense Head:", name: "expense_head",type:"readonly"},--}}
            {{--        // {label: "Expense Name:", name: "name",type:"readonly"},--}}
            {{--        // {label: "Date:", name: "expense_date",type:"datetime"},--}}
            {{--        // {label: "Expense Amount:", name: "amount",type:"readonly"},--}}
            {{--        // {label: "Expense Description:", name: "description"},--}}
            {{--    ]--}}
            {{--});--}}

            $('#installmentplan thead tr').clone(true).appendTo( '#installmentplan thead' );
            $('#installmentplan thead tr:eq(1) th').each( function (i) {
                if (i==0) {
            ...
            var table=  $('#installmentplan').DataTable({
                dom: "Blfrtip",
                "processing": true,
                "serverSide": true,
                "ajax": '{{ url('profit_reconciliation/dt_get_all_reconciliations') }}',
                    ...
                    {'extend': 'print', className: 'm-btn--square  btn-outline-primary',
                        ...
                        title: function(){
                            var printTitle = '' +
                                ...
                                '                                        <h1 class=" mr-2" style="font-family: sans-serif;text-decoration: underline">Profit Reconciliation Report</h5>\n' +
                                '                                    </div>\n' +
                                '                                    <div class="col-md-6">\n' +
                ...
                columns:[
                    ...
                    {   data: "closing_date_from",
                        render: function ( data, type, row ) {
                            return row.closing_date_from + '-' + row.closing_date_to ;
                        }
                    },
                    {data:'profit_loss','name':'profit_loss'},
                    {data:'total_profit','name':'total_profit'},
                    {data:'total_reconciled','name':'total_reconciled'},
                    {data:'name','name':'name'},
                    {data:'amount_reconciled','name':'amount_reconciled'},
                    {data:'reconcile_description','name':'reconcile_description'},
            ...
            {{--$('.loaddatabydate').click(function (e) {--}}
            {{--    var min=$('.min').val();--}}
            {{--    var max=$('.max').val();--}}
            {{--    console.log(table);--}}
            {{--    // table.ajax.load('getdatabydateledger?min='+min+'&'+'max='+max).draw();--}}
            {{--    table.ajax.url("{{url('expense/dtgetexpensebydatedetails')}}?min="+min+'&'+'max='+max).load();--}}
            {{--});--}}

            $('#installmentplan').on('click', 'tbody td:not(:first-child)', function (e) {
                editor.inline(this);
            });
