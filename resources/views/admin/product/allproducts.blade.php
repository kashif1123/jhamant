...
@section('content')
    ...
    <input type="hidden" value="{{credentials()->company_name}}" id="company_name">
    ...
    <script>
        ...
        $(document).ready(function (e) {
            ...
            editor =  new $.fn.DataTable.Editor({
                ajax: "{{ url('product/dtpostshowproducts') }}",
                table: "#products",
                display: "bootstrap",
                idSrc:'id',
                keys:true,
                fields: [
                    ...
                    {label: "Barcode:", name: "barcode"},
                    {
                        ...
                        opts:{
                            ajax:{
                                ...
                                dataType: 'json',
                                delay: 250,
                                url: " {{ url('/get_categories_for_products') }}",
                    ...
                    },
                    {
                        ...
                        opts:{
                            ajax:{
                                ...
                                dataType: 'json',
                                delay: 250,
                                url: " {{ url('/get_companies_for_products') }}",
                                data: function (params) {
                                    // console.log($(this).closest('tr')).data();
                                    // var mydata=table.row($(this).closest('tr')).data();
                                    // console.log(mydata);
                    ...
                    {label: "Company:", name: "asdf",type:"readonly"},
                    {label: "Purchase Price:", name: "purchase_price"},
                    {label: "Sale Price:", name: "sale_price"},
                    {label: "Sale %:", name: "sale_percentage"},
                    {label: "Quantity Available:", name: "total_qty",type:"readonly"},
                ]
            });
            ...
            var table=  $('#products').DataTable({
                ...
                "processing": true,
                "serverSide": true,
                "ajax": '{{ url('product/dtgetshowproducts') }}',
                    ...
                    {'extend': 'print', className: 'm-btn--square  btn-outline-primary',
                        customize: function ( win ) {
                            ...
                            $(win.document.body).find( 'table' )
                                .prepend (
                                    ...
                                    '<th style="text-align: center;">Avg Purchase Price</th>' +
                                    '<th style="text-align: center;">Sale Price</th>' +
                                    '<tH style="text-align: center;">Quantity Available</tH>' +
                                    '<tH style="text-align: center;">Sale %</tH>' +
                                    '<tH style="text-align: center;">Print Barcode</tH>' +
                                    '</tr>'
                                );
                        ...
                        title: function(){
                            var printTitle = '' +
                                ...
                                '                                        <h1 class=" mr-2" style="font-family: sans-serif;text-decoration: underline">Stock Report</h5>\n' +
                                '                                    </div>\n' +
                                '                                    <div class="col-md-6">\n' +
                ...
                columns:[
                    ...
                    {data:'c_name','name':'c_name',editField:'category_id'},
                    {data:'company','name':'company',editField:'company_id'},
                    {data:'unit_name','name':'unit_name',editField:'unit_id'},
                    {data:'purchase_price','name':'purchase_price'},
                    {data:'avg_purchase_price','name':'avg_purchase_price'},
                ...
                "footerCallback": function(row, data, start, end, display) {
                    var api = this.api();
                    var p_price= api.column( 6, {page:'current'} ).data().sum();
                    var qty= api.column( 9, {page:'current'} ).data().sum();
                    var s_price= api.column( 8, {page:'current'} ).data().sum();
                    $('.p_price').html(p_price);
                    $('.qty').html(qty);
                    $('.s_price').html(s_price);
                }
            });
            ...
            function smallBarocde(name,price){
                ...
                html+='<div style="padding:1px !important;"  >' +
                    ...
                    '<b style="font-family: sans-serif" >'+company_name+'</b><span style="font-size: 8pt !important;float: right; font-family: sans-serif;margin-bottom:0px">Rs:'+parseFloat(price)+' </span><br>'+
